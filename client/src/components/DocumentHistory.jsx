...
export default function DocumentHistory({ onSelect, refreshKey }) {
  ...
  if (contracts.length === 0) {
    return (
      <p className="rounded-xl border border-dashed border-slate-200 px-4 py-6 text-center text-sm text-slate-400 dark:border-slate-800">
        No documents analyzed yet. Upload a contract above to get started.
      </p>
    );
  }
    ...
    <div className="space-y-2">
      {contracts.map((c) => (
        ...
        >
          ...
          <RiskBadge level={c.overallRiskScore} />
        </button>
      ))}
    </div>
  );
